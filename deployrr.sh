...
load_catalog() {
    ...
    define_app boxarr     "Boxarr"      "lscr.io/linuxserver/bookstack:latest"   "ARR Suite"  "6875:80"
    ...
    define_app overseerr  "Overseerr"  "lscr.io/linuxserver/overseerr:latest" "Request & Tools"  "5056:5055"
...
ALL_APPS=(
    ...
    # Request & Monitoring
    seerr overseerr ombi requestrr tautulli flaresolverr
...
add_service_doplarr() {
    local f="${COMPOSE_FILE}"
    {
        ...
        echo "      - OVERSEERR__API=           # <-- Set your Overseerr API key"
...
# Load app catalog from generated catalog.sh
CATALOG_SH="${SCRIPT_DIR}/apps/catalog.sh"
if [[ -f "${CATALOG_SH}" ]]; then
    # shellcheck source=/dev/null
    source "${CATALOG_SH}"
else
    log_err "main" "catalog.sh not found at ${CATALOG_SH} — run: python3 scripts/gen_catalog_sh.py"
fi
...
"The following will be deployed:\n\n${app_list}\nProceed?"; then
    ...
    for id in "${apps[@]}"; do
        ...
        if ${is_local}; then
            log INFO "Pull SKIP (local image) [${idx}/${total}] ${id}  image=${image}"
            ok_pull+=("${id}")
            printf '%d\n# [SKIP — local] %s\n' "${pct}" "${label}" >&9
            continue
        fi
...
deploy_by_category() {
    ...
    cat=$(d_menu "Deploy by Category" "Select a category:" \
        ...
        "Request & Tools"  "Jellyseerr, Tautulli, Ombi, etc." \
...
tailscale_menu() {
    ...
    if command -v tailscale &>/dev/null; then
        ...
        d_msgbox "Tailscale Not Found" \
"Tailscale is not installed or not running.\n\nInstall options:\n  1. Deploy via: media → Deploy → Search → 'tailscale'\n  2. Install natively: curl -fsSL https://tailscale.com/install.sh | sh"
        return
    fi
